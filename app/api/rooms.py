...
async def create_dm(dm_in: DMCreate, current_user: UserModel = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    # Check if a private room exists with exactly these two members
    my_rooms = db.query(RoomMemberModel.room_id).filter(RoomMemberModel.user_id == current_user.id).subquery()
    target_rooms = db.query(RoomMemberModel.room_id).filter(RoomMemberModel.user_id == target_user_id).subquery()
...
async def create_room(room_in: RoomCreate, current_user: UserModel = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    room_member = RoomMemberModel(user_id=current_user.id, room_id=room.id, role="admin")
    db.add(room_member)
    db.commit()
...
async def read_rooms(
    ...
    if is_private is not None:
        ...
        # Show only rooms the user is a member of
...
async def read_room(room_id: str, current_user: UserModel = Depends(get_current_active_user), db: Session = Depends(get_db)):
...
async def update_room(room_id: str, room_in: RoomUpdate, admin_member: RoomMemberModel = Depends(is_room_admin), db: Session = Depends(get_db)):
    room = db.query(RoomModel).filter(RoomModel.id == room_id).first()
    if not room:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Room not found")
...
async def delete_room(room_id: str, admin_member: RoomMemberModel = Depends(is_room_admin), db: Session = Depends(get_db)):
    room = db.query(RoomModel).filter(RoomModel.id == room_id).first()
    if not room:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Room not found")
...
async def join_room(room_id: str, current_user: UserModel = Depends(get_current_active_user), db: Session = Depends(get_db)):
    room = db.query(RoomModel).filter(RoomModel.id == room_id).first()
    if not room:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Room not found")
    ...
    # Add user as member
    new_member = RoomMemberModel(user_id=current_user.id, room_id=room_id, role="member")
    db.add(new_member)
    db.commit()
...
async def join_room_by_code(join_code: str, current_user: UserModel = Depends(get_current_active_user), db: Session = Depends(get_db)):
    ...
    # Add user as member
    new_member = RoomMemberModel(user_id=current_user.id, room_id=room.id, role="member")
    db.add(new_member)
    db.commit()
...
async def leave_room(room_id: str, current_user: UserModel = Depends(get_current_active_user), db: Session = Depends(get_db)):
    room = db.query(RoomModel).filter(RoomModel.id == room_id).first()
    if not room:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Room not found")
    ...
    if room.created_by == current_user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Room creator cannot leave. Delete the room instead.")
...
async def promote_member_to_admin(room_id: str, user_id: str, admin_member: RoomMemberModel = Depends(is_room_admin), db: Session = Depends(get_db)):
...
async def demote_admin_to_member(room_id: str, user_id: str, admin_member: RoomMemberModel = Depends(is_room_admin), db: Session = Depends(get_db)):
    if str(admin_member.user_id) == user_id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot change your own role.")
...
async def remove_member(room_id: str, user_id: str, admin_member: RoomMemberModel = Depends(is_room_admin), db: Session = Depends(get_db)):
    if str(admin_member.user_id) == user_id:
...
async def transfer_ownership(room_id: str, transfer_request: RoomOwnershipTransfer, admin_member: RoomMemberModel = Depends(is_room_admin), db: Session = Depends(get_db)):
    ...
    # Promote the new owner and demote the old one
